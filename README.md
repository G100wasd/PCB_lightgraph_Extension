...
<p align="center">
  ...
  <strong> 直接点击导入图纸至 EDA 中！</strong>
</p>

## 界面 & 特殊功能

<p align="center">
  <img src="images/interface-and-features.png" alt="PCB lightgraph Extension 界面与完整功能预览" width="100%">
</p>

- **直接导入 EDA**：可选单层或全部生产层，按指定坐标转换为当前 PCB 中的图元，无需先落地导出文件。
- **工程可携带**：完全兼容保存和导入 `.pcblg` 工程包，保存原图、参数、视图和灯条数据，并与安装版工程格式兼容。
- **导出兼容保护**：默认在导出 PNG 时加入边角兼容标记，以降低部分 EDA 对空白／透明图层误判的风险；可在“选项”菜单关闭。
- **可选择UI风格**：用户可自主选择使用EDA的UI风格还是便携版UI风格。
## 快速开始

### 安装扩展

1. 前往 [Releases](https://github.com/G100wasd/PCB_lightgraph_Extension/releases) 下载最新的 `.eext` 扩展包。
2. 在嘉立创 EDA 专业版中顶部栏的 `高级` 打开扩展管理页面，点击`导入`选中该 `.eext` 扩展包并完成安装。
3. 打开或新建一个 PCB，使用顶部菜单的 `PCB lightgraph → Open` 启动工具。

> [!WARNING]
> 在 PCB 编辑器中打开时，可直接把生成的生产层导入当前 PCB ，在首页或原理图编辑器中同样可以打开工具，但 **不会执行** EDA 导入！
...
> [!CAUTION]
> 图片分层结果受原始素材的色彩、对比度和细节影响，所以在导入 EDA 或实际制板前，**请在 PCB 中检查图元、层别、尺寸与可制造性**。
...
本 EDA 插件是由 [Codex](https://github.com/openai/codex) 辅助移植和开发的一款将 2D 插画转换为 PCB 分层图纸的开源HTML工具（以下简称“本工具”），**主要面向学习与个人创作**。
